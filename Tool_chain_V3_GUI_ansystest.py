...
#Bounds
	...
	def runMethod(self):
		...
		num = int(self.lbl_num_Edit.text())
		...
		method = self.lbl_method_Edit.currentText()
		variablenames = self.lbl_variable_names_Edit.text()
		variablenames = variablenames.split(',')
		#rbf
		k_fold = int(self.lbl_Rbf_k_fold_Edit.text())
		names = [y for y in self.lbl_GSA_names_Edit.text().split(',')]
		...
		if method == 'sobol':
		#Sobol
			skip = self.lbl_sobol_skip_Edit.text() 
			if skip != '' :
				skip = int(skip)
